...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-lg border-b border-gray-100 shadow-sm transition-all">
        ...
        <div className="flex justify-between h-20 items-center">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex items-center gap-6">
              ...
              <Link to="#" className="text-sm font-medium text-gray-600 hover:text-indigo-600 transition-colors">New Arrivals</Link>
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            {user?.isAdmin && (
              <Link to="/admin" className="hidden sm:flex items-center gap-1 text-xs font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 px-3 py-1.5 rounded-full uppercase tracking-wide">
                <ShieldCheck size={14} />
                Admin Panel
              </Link>
            )}
              ...
              {user ? (
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-2 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all transform origin-top-right scale-95 group-hover:scale-100 z-50">
                    ...
                    <Link to="/profile" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Orders</Link>
                    <button 
                ...
                <Link to="/login" className="flex items-center gap-2 text-sm font-bold text-white bg-gray-900 hover:bg-indigo-600 px-5 py-2.5 rounded-full transition-all shadow-lg shadow-gray-200 hover:shadow-indigo-200">
                  Login
                </Link>
              )}
            </div>
